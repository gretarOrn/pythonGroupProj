...
def show_filter(paths, destPath):
    seasonReg = re.compile("[Ss](eason)[s]*\s*.*[0-9]{0,3}|(SEASON)[S]*\s*.*[0-9]{0,3}|(season)[s]*\s*.*[0-9]{0,3}|[Ss](er)[íi](a)\s*.*[0-9]{0,3}|\\\\[Ss]*[0-9]{1,2}\\\\|[Ss](erie)s*|\\\\\d{1,2}\\\\")
    singleShowReg = re.compile("[sS][0-9]{1,2}\s*[eE][0-9]{1,2}|\D+[0-9]{1,2}[xX][0-9]{1,2}\D+")
    secondSingleShowReg = re.compile("[\.\-\_\s]*[0-9]{3}[\.\-\_\s]*")
